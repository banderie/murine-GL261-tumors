function err=gbm_minimizerhyp2(par,ic,mouse)
...
p.tspan = [0,0.44,0.6,0.72,0.84,1]; % linspace(start,finish, 4);  % fixed steps are required for ensemble simulations
%p.tspan=[0.72,0.84,1];
...
% G3_M2 = 4;
% G3_M3 = 5;
p.manual_initial_condition = mouse+2;
...
% [~,~,initbrain,inittumor,~,~,~,~] = mask(filenames);
%  figure;
...
tumor_bg4 = squeeze(ensemble_tumor(end-1,:,:,:)); m.tumor4(m.tumor4>0)=1;
...
figure
imagesc(squeeze(plot_tumor1(:,:,p.zslice)));
caxis([0 2.03])
colormap(vertcat(gray(250),jet(256)));
title('Time point 1')
% % 
...
% 
 err=5-intersection1/union1-intersection/union-intersection2/union2-intersection3/union3-intersection4/union4
 ...
 par
 if mouse==3
     err=3-intersection/union-intersection4/union4-intersection3/union3
 end
%err=2-intersection/union-intersection4/union4
